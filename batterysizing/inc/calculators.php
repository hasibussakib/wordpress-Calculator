...
function batterysizing_get_calculators() {
	$calculators = array(
		...
		array(
			...
			'tagline'          => __( 'How many hours will your battery run a given load?', 'batterysizing' ),
			'icon'             => 'timer',
			'engine'           => 'runtime',
			'meta_title'       => 'Battery Runtime Calculator — How Long Will a Battery Last?',
			'meta_description' => 'Free battery runtime calculator: enter voltage, Ah, load watts and depth of discharge to see exactly how many hours and minutes your battery will last.',
			'intro'            => '<p>Enter your battery specs and the load it powers. The runtime calculator accounts for usable capacity (depth of discharge) and inverter losses, so the result matches real-world backup time — not the optimistic number on the battery label.</p>',
			'formula'          => 'Runtime (h) = ( V × Ah × DoD ÷ 100 × η ÷ 100 ) ÷ Load (W)',
			'steps'            => array(
				'Enter the battery voltage (V) and capacity (Ah) from the label.',
				'Enter the total load in watts the battery has to power.',
				'Pick your depth of discharge — 50% for lead-acid, 80–100% for lithium.',
				'Read the runtime in hours and minutes, plus usable energy in Wh.',
			),
			'faq'              => array(
				array(
					'q' => 'Why is my real runtime shorter than the calculated one?',
					'a' => 'Battery capacity is rated at a slow 20-hour discharge. Pulling high current makes lead-acid batteries deliver less (Peukert effect). Heat, age and low inverter efficiency shave off more. Expect 10–25% less than the ideal calculation for older lead-acid batteries.',
				),
				array(
					'q' => 'How long will a 12V 100Ah battery run a 100W appliance?',
					'a' => 'At 50% depth of discharge (lead-acid) and 85% inverter efficiency: 12 × 100 × 0.5 × 0.85 ÷ 100 ≈ 5.1 hours. The same battery in LiFePO4 chemistry at 90% DoD gives ≈ 11 hours.',
				),
				array(
					'q' => 'Should I include inverter efficiency?',
					'a' => 'Yes, whenever the load runs through an inverter (AC appliances). Inverters waste 5–15% as heat. For pure DC loads you can set efficiency to 100%.',
				),
			),
